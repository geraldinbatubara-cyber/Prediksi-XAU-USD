...
def _fixed_delay_live_signal(
    ...
    if raw.empty:
        ...
        return None, {
            **empty_state,
            **diagnostics,
            "Status": "MENUNGGU PEMETAAN M1" if has_daily_signal else "MENUNGGU SINYAL HARIAN",
            "Detail": (
            ...
            "Kode diagnosis": "DAILY_SIGNAL_OUTSIDE_M1_COVERAGE" if has_daily_signal else "NO_DAILY_V1_SIGNAL",
        }
...
def _buy_specialist_observation(
    ...
    raw = _raw_model_probabilities(model_bundle["estimators"], latest)["Hierarchical Ensemble"]
    ...
    m15_row = entry_features.reindex(pd.DatetimeIndex([evaluation_time]), method="ffill")
    m15_buy = False
    if not m15_row.empty:
        ...
        if all(pd.notna(values.get(column)) for column in required):
            m15_buy = bool(
                float(values["price"]) > float(values["m15_fast"]) > float(values["m15_slow"])
                and float(values["m15_momentum"]) > 0
            )
    regime = _buy_specialist_regime(model_bundle, data, evaluation_time, latest.iloc[0])
    return {
...
def _buy_specialist_v4_signal(
    ...
    live_observation = _buy_specialist_observation(data, model_bundle, pd.Timestamp(data.index.max()))
    ...
    observation = _buy_specialist_observation(data, model_bundle, confirmation_time)
    ...
    probability = {"trend": observation["P(trend)"], "up": observation["P(BUY)"]}
