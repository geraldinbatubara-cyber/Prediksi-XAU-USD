...
V1_SELL_SPECIALIST_V6_VERSION = (
    "optimizer-v1-sell-specialist-bear-regime-setup-2022-2026h1-v6"
)
V1_SELL_SPECIALIST_V6_PATH = Path(
    "data/precomputed/v1_sell_specialist_v6.pkl.b64"
)
V1_SELL_SPECIALIST_V7_VERSION = (
    "optimizer-v1-sell-specialist-bear-event-exhaustion-2022-2026h1-v7"
)
V1_SELL_SPECIALIST_V7_PATH = Path(
    "data/precomputed/v1_sell_specialist_v7.pkl.b64"
)
V1_SIDEWAYS_SPECIALIST_VERSION = (
    "optimizer-v1-sideways-specialist-range-mean-reversion-2022-2026h1-v1"
)
V1_SIDEWAYS_SPECIALIST_PATH = Path(
    "data/precomputed/v1_sideways_specialist.pkl.b64"
)
V1_SIDEWAYS_SPECIALIST_V2_VERSION = (
    "optimizer-v1-sideways-specialist-persistence-hazard-2022-2026h1-v2"
)
...
V1_SIDEWAYS_SPECIALIST_V4_VERSION = (
    "optimizer-v1-sideways-specialist-event-hazard-2022-2026h1-v4"
)
...
V1_SIDEWAYS_SPECIALIST_V7_VERSION = (
    "optimizer-v1-sideways-specialist-mtf-native-persistence-2022-2026h1-v7"
)
...
@st.cache_resource
def load_precomputed_v1_entry_outcome(backtest_version: str):
    if not V1_ENTRY_OUTCOME_PATH.exists():
        return None
    try:
        saved = pickle.loads(
            base64.b64decode(V1_ENTRY_OUTCOME_PATH.read_text(encoding="ascii"))
        )
        if saved.get("version") == backtest_version:
            return saved["payload"]
    except Exception:
...
def load_precomputed_v1_entry_quality_path(backtest_version: str):
    ...
    try:
        saved = pickle.loads(
            base64.b64decode(V1_ENTRY_QUALITY_PATH_PATH.read_text(encoding="ascii"))
        )
        if saved.get("version") == backtest_version:
            return saved["payload"]
    except Exception:
...
@st.cache_resource
def load_precomputed_v1_fixed_delay(backtest_version: str):
    if not V1_FIXED_DELAY_PATH.exists():
        return None
    try:
        saved = pickle.loads(
...
@st.cache_resource
def load_precomputed_v1_fixed_delay_quality(backtest_version: str):
    if not V1_FIXED_DELAY_QUALITY_PATH.exists():
        return None
    try:
        saved = pickle.loads(
            base64.b64decode(
...
@st.cache_resource
def load_precomputed_v1_sell_specialist_v6(backtest_version: str):
    if not V1_SELL_SPECIALIST_V6_PATH.exists():
        return None
    try:
        saved = pickle.loads(
            base64.b64decode(
                V1_SELL_SPECIALIST_V6_PATH.read_text(encoding="ascii")
            )
        )
        if saved.get("version") == backtest_version:
            return saved["payload"]
...
@st.cache_resource
def load_precomputed_v1_sell_specialist_v7(backtest_version: str):
    if not V1_SELL_SPECIALIST_V7_PATH.exists():
        return None
    try:
        saved = pickle.loads(
            base64.b64decode(
                V1_SELL_SPECIALIST_V7_PATH.read_text(encoding="ascii")
            )
        )
        if saved.get("version") == backtest_version:
            return saved["payload"]
...
@st.cache_resource
def load_precomputed_v1_sideways_specialist(backtest_version: str):
    if not V1_SIDEWAYS_SPECIALIST_PATH.exists():
        return None
    try:
        saved = pickle.loads(
            base64.b64decode(
                V1_SIDEWAYS_SPECIALIST_PATH.read_text(encoding="ascii")
            )
        )
        if saved.get("version") == backtest_version:
            return saved["payload"]
...
@st.cache_resource
def load_precomputed_v1_sideways_specialist_v2(backtest_version: str):
    if not V1_SIDEWAYS_SPECIALIST_V2_PATH.exists():
        return None
    try:
        saved = pickle.loads(
            base64.b64decode(
...
@st.cache_resource
def load_precomputed_v1_sideways_specialist_v4(backtest_version: str):
    if not V1_SIDEWAYS_SPECIALIST_V4_PATH.exists():
        return None
    try:
        saved = pickle.loads(
            base64.b64decode(
...
@st.cache_resource
def load_precomputed_v1_sideways_specialist_v7(backtest_version: str):
    if not V1_SIDEWAYS_SPECIALIST_V7_PATH.exists():
        return None
    try:
        saved = pickle.loads(
            base64.b64decode(
...
@st.cache_resource
def load_precomputed_v1_sideways_specialist_v9(backtest_version: str):
    if not V1_SIDEWAYS_SPECIALIST_V9_PATH.exists():
        return None
    try:
        saved = pickle.loads(
            base64.b64decode(
                V1_SIDEWAYS_SPECIALIST_V9_PATH.read_text(encoding="ascii")
            )
        )
        if saved.get("version") == backtest_version:
            return saved["payload"]
...
def render_dashboard(
    ...
    v1_daily_comparison = compare_locked_prediction_to_live(
    ...
    with col3:
        ...
        st.metric(
            "Prediksi harian terkunci",
            f"${v1_daily_prediction:,.2f}",
        ...
        st.metric(
            ...
            (
                None
                if pd.isna(v1_daily_comparison["live_difference"])
                else f"{v1_daily_comparison['live_difference']:+,.2f} vs prediksi"
            ),
        )
    ...
    comparison = pd.DataFrame(
        [
            ...
            },
            {
                ...
                "Output utama": f"${v1_daily_prediction:,.2f}",
                "Arah / Sinyal": signal_label,
                "Status kalibrasi": "Snapshot harian terkunci",
                "Data model": _format_date(v1_prediction.get("latest_date")),
                "Perubahan vs basis live": v1_daily_prediction - forecast_basis,
...
def _render_v1_entry_outcome_tab(payload) -> None:
    st.subheader("v1 Balanced Entry Outcome Lab")
    if payload is None:
        st.warning("Hasil Entry Outcome Lab belum tersedia pada artefak precomputed.")
        return

    methodology = payload["methodology"]
    decision = payload["decision"]
    validation = payload["validation_metrics"].iloc[0]
    selected_economic = (
        payload["economic"]
        .set_index("Strategi")
        .loc["v1 Probability-Gated Entry"]
    )

    st.warning(
        "**Eksperimen terisolasi:** v1 Exact Baseline, Balanced Entry Frozen, ledger, dan Live Trading "
        "tidak diubah. Probability gate ini belum dipromosikan."
    )
    if decision["Lulus seluruh kriteria"]:
        st.success(
            "**Status historis: LULUS.** Kandidat tetap harus menjalani forward paper shadow."
        )
    else:
        st.error(
            "**Status historis: BELUM LULUS.** Hasil ekonomi sedikit membaik, tetapi probabilitas belum "
            "mengalahkan base rate dan selection masih memakai fallback."
        )
    st.info(
        f"Model: **{methodology['Selected model']}** | Threshold: "
        f"**{methodology['Selected threshold']:.1%}** | Probabilitas impas teoritis: "
        f"**{methodology['Theoretical break-even probability']:.1%}** | "
        f"Kriteria lolos: **{decision['Jumlah kriteria lolos']}/{len(decision) - 4}**."
    )
    st.caption(methodology["Direction normalization"])
    st.caption(methodology["Caveat"])

    m1, m2, m3, m4, m5, m6 = st.columns(6)
    m1.metric("Brier improvement", f"{validation['Brier improvement (%)']:+.2f}%")
    m2.metric("PR-AUC", f"{validation['PR-AUC']:.3f}")
    m3.metric("Calibration error", f"{validation['Calibration error']:.3f}")
    m4.metric("Growth gated", f"{selected_economic['Growth (%)']:+.2f}%")
    m5.metric("Profit factor", f"{selected_economic['Profit factor']:.3f}")
    m6.metric("Max drawdown", f"{selected_economic['Max drawdown (%)']:.2f}%")

    st.markdown("**Perbandingan Model pada Expanding Walk-Forward 2025**")
    st.dataframe(
        payload["model_summary"].style.format(precision=3),
        use_container_width=True,
        hide_index=True,
    ...
    st.markdown("**Pemilihan Threshold pada Prediksi OOF Development**")
    st.dataframe(
        payload["threshold_development"].style.format(
            {
                "Threshold": "{:.1%}", "Equity akhir": "${:,.2f}",
                "Growth (%)": "{:+.2f}%", "Max drawdown": "${:,.2f}",
                "Max drawdown (%)": "{:.2f}%", "Profit factor": "{:.3f}",
                "Transaksi": "{:.0f}", "Win rate (%)": "{:.1f}%",
                "Retensi entry (%)": "{:.1f}%", "Retensi net profit (%)": "{:.1f}%",
            },
            na_rep="-",
        ),
        use_container_width=True,
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("**Validation Probabilitas 2026H1**")
        st.dataframe(
            payload["validation_metrics"].style.format(precision=3),
            use_container_width=True,
            hide_index=True,
        )
    with c2:
        st.markdown("**Kalibrasi per Probability Bucket**")
        st.dataframe(
            payload["calibration"].style.format(
                {
    ...
    c3, c4 = st.columns(2)
    with c3:
        st.markdown("**Distribusi Outcome**")
        st.dataframe(
        ...
        st.dataframe(
            payload["direction_audit"].style.format(precision=3),
            use_container_width=True,
            hide_index=True,
        )
    with c4:
        ...
        st.markdown("**Ablation Feature Group**")
        st.dataframe(
            payload["ablation"].style.format(precision=3),
            use_container_width=True,
            hide_index=True,
        )

    with st.expander("Stress spread, slippage, delay, dan threshold"):
        st.markdown("**Spread dan Slippage**")
        st.dataframe(payload["stress"].style.format(precision=3), use_container_width=True, hide_index=True)
        st.markdown("**Delay Entry M1**")
        st.dataframe(payload["delay_stress"].style.format(precision=3), use_container_width=True, hide_index=True)
        st.markdown("**Threshold Sensitivity pada Validation**")
        st.dataframe(
            payload["threshold_sensitivity"].style.format(precision=3),
            use_container_width=True,
            hide_index=True,
        )

    with st.expander("Audit event validation"):
        st.dataframe(
            payload["validation_events"].style.format(
                {"target": "{:.0f}", "probability": "{:.1%}", "hours_to_outcome": "{:.1f}"}
            ),
            use_container_width=True,
        )
...
def _render_v1_entry_quality_path_tab(payload) -> None:
    ...
    if payload is None:
        st.warning("Hasil Path-Aware v3 belum tersedia pada artefak precomputed.")
        return
    ...
    economic = (
        payload["economic"]
        .set_index("Strategi")
        .loc["v1 Entry Quality Path-Aware"]
    )

    st.warning(
        "**Eksperimen terisolasi:** v1 Exact Baseline, Balanced Entry, ledger, dan Live Trading "
        "tetap terkunci. MFE/MAE v3 berhenti pada barrier pertama."
    )
    if decision["Lulus seluruh kriteria"]:
        st.success("**Status: LULUS HISTORIS.** Kandidat masih wajib menjalani prospective paper shadow.")
    else:
        st.error(
            "**Status: BELUM LULUS.** Gate tidak berhasil membedakan winner dan loser pada "
            "historical confirmation 2026H1."
        )
    st.info(
        f"Model: **{methodology['Selected model']}** | EV minimum: "
        f"**USD {methodology['Selected EV minimum']:.2f}** | P(TP) minimum: "
    ...
    st.caption(methodology["Development"])
    st.caption(f"Purge: {methodology['Purge']}")
    st.caption(methodology["Caveat"])

    m1, m2, m3, m4, m5, m6 = st.columns(6)
    m1.metric("Brier improvement", f"{confirmation['Brier improvement (%)']:+.2f}%")
    m2.metric("TP ROC-AUC", f"{confirmation['ROC-AUC']:.3f}")
    m3.metric("Transaksi", f"{economic['Transaksi']:.0f}")
    m4.metric("Growth", f"{economic['Growth (%)']:+.2f}%")
    m5.metric("Profit factor", f"{economic['Profit factor']:.3f}")
    m6.metric("Max drawdown", f"{economic['Max drawdown (%)']:.2f}%")
    ...
    st.markdown("**Perbandingan Model pada 12 Expanding Quarterly Folds**")
    st.dataframe(
        payload["model_summary"].style.format(precision=3),
        use_container_width=True,
        hide_index=True,
    ...
    st.markdown("**Pemilihan Rule OOF Development**")
    st.dataframe(
        payload["threshold_development"].style.format(
            {
                "EV minimum": "${:,.2f}",
    ...
    st.markdown("**Apakah Gate Memilih Entry yang Tepat?**")
    st.dataframe(
        payload["classification_audit"].style.format(
            {
                "Proporsi dari seluruh event (%)": "{:.1f}%",
                "Median probability (%)": "{:.1f}%",
                "Median MFE": "${:,.2f}",
                "Median MAE": "${:,.2f}",
            },
            na_rep="-",
    ...
    st.caption(
        f"Retensi net profit terhadap Balanced Entry: "
        f"{payload['profit_retention']:.1f}%."
    )
    ...
    with c4:
        st.markdown("**Path MFE/MAE sampai Barrier Pertama**")
        st.dataframe(
            payload["path_excursions"].style.format(
                {
                    "Median MFE": "${:,.2f}",
                    "P90 MFE": "${:,.2f}",
                    "Median MAE": "${:,.2f}",
...
def _render_v1_fixed_delay_tab(payload) -> None:
    st.subheader("v1 Fixed Delay 5m Robustness Lab")
    if payload is None:
        st.warning("Hasil Fixed Delay 5m belum tersedia pada artefak precomputed.")
        return

    methodology = payload["methodology"]
    decision = payload["decision"]
    confirmation = (
        payload["confirmation_economic"]
        .set_index("Strategi")
        .loc["Fixed Delay 5m Validated"]
    )
    development = (
        payload["development_economic"]
        .set_index("Strategi")
        .loc["Fixed Delay 5m Validated"]
    )
    ...
    st.warning(
        "**Eksperimen terisolasi:** v1 Exact Baseline, Balanced Entry, ledger, dan Live Trading "
        "tetap terkunci. Delay lima menit diuji tanpa pencarian parameter dan tanpa micro gate."
    )
    if decision["Lulus seluruh kriteria"]:
        st.success("**Status: LULUS HISTORIS.** Kandidat tetap memerlukan prospective paper shadow.")
    else:
        st.error(
            "**Status: BELUM LULUS ROBUSTNESS.** Historical confirmation 2026H1 kuat, "
            "tetapi development agregat belum memenuhi batas drawdown dan profit factor."
        )
    st.info(
        f"Rule: **{methodology['Rule']}** | Fold profitable: "
        f"**{decision['Jumlah fold profitable']}/12** | Kriteria lolos: "
        f"**{decision['Jumlah kriteria lolos']}/{decision['Jumlah kriteria']}**."
    )
    st.caption(methodology["Barrier rule"])
    st.caption(methodology["Spread rule"])
    st.caption(methodology["Caveat"])

    m1, m2, m3, m4, m5, m6 = st.columns(6)
    m1.metric("Growth 2026H1", f"{confirmation['Growth (%)']:+.2f}%")
    m2.metric("PF 2026H1", f"{confirmation['Profit factor']:.3f}")
    m3.metric("DD 2026H1", f"{confirmation['Max drawdown (%)']:.2f}%")
    m4.metric("Transaksi 2026H1", f"{confirmation['Transaksi']:.0f}")
    m5.metric("PF development", f"{development['Profit factor']:.3f}")
    m6.metric("DD development", f"{development['Max drawdown (%)']:.2f}%")
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("**Development Agregat 2022-2025**")
        st.dataframe(
            payload["development_economic"].style.format(metric_formats, na_rep="-"),
            use_container_width=True,
            hide_index=True,
        )
    with c2:
        st.markdown("**Historical Confirmation 2026H1**")
        st.dataframe(
            payload["confirmation_economic"].style.format(metric_formats, na_rep="-"),
            use_container_width=True,
            hide_index=True,
        )
    ...
    st.markdown("**Sensitivitas Delay 4-6 Menit**")
    st.dataframe(
        payload["delay_sensitivity"].style.format(metric_formats, na_rep="-"),
        use_container_width=True,
        hide_index=True,
    )
    st.markdown("**Audit Sinyal yang Dibatalkan Sebelum Entry**")
    st.dataframe(
        payload["cancellation_audit"],
        use_container_width=True,
        hide_index=True,
    )
    ...
    c3, c4 = st.columns(2)
    with c3:
        st.markdown("**Audit Regime 2026H1**")
        st.dataframe(
            payload["regime_audit"].style.format(metric_formats, na_rep="-"),
            use_container_width=True,
            hide_index=True,
        )
    with c4:
    ...
    monte_carlo = payload["selected_monte_carlo_summary"]
    concentration = payload["profit_concentration"].iloc[0]
    st.info(
        f"Monte Carlo 10.000 run: probabilitas equity akhir di bawah modal "
        f"**{monte_carlo['Probabilitas equity akhir < modal awal (%)']:.2f}%**, "
        f"equity P5 **USD {monte_carlo['Monte Carlo equity P5']:,.2f}**. "
        f"Konsentrasi gross profit dua transaksi terbesar "
        f"**{concentration['Konsentrasi top 2 (%)']:.2f}%**."
    )

    with st.expander("Detail fold, bulanan, stress, dan audit event"):
        st.markdown("**12 Quarterly Folds 2023-2025**")
        st.dataframe(
        ...
        st.dataframe(
            payload["monthly"].style.format(precision=3),
            use_container_width=True,
            hide_index=True,
        )
        st.markdown("**Stress Spread dan Slippage**")
        ...
        st.markdown("**Event Fixed Delay 5 Menit**")
        st.dataframe(
            payload["confirmation_events"].style.format(precision=3),
            use_container_width=True,
        )
...
def _render_v1_fixed_delay_quality_tab(payload) -> None:
    st.subheader("v1 Fixed Delay Quality Guard Lab")
    if payload is None:
        st.warning("Hasil Fixed Delay Quality Guard belum tersedia pada artefak precomputed.")
        return

    methodology = payload["methodology"]
    ranking = payload["ranking"]
    winner = ranking.iloc[0]
    control = ranking.set_index("Kandidat").loc["Fixed Delay 5m Control"]
    st.warning(
        "**Eksperimen terisolasi:** enam kandidat hanya mengubah filter entry setelah Fixed Delay 5m. "
        "Baseline v1, paper live Fixed Delay, ledger, TP/SL, lot, biaya, dan exit tidak berubah."
    )
    if bool(winner["Lulus"]):
        st.success(
            f"**Status: LULUS DEVELOPMENT.** {winner['Kandidat']} memenuhi seluruh gerbang "
            "dan layak masuk prospective paper shadow."
        )
    else:
        st.error(
            f"**Status: BELUM LULUS.** {winner['Kandidat']} menjadi kandidat terbaik dengan "
            f"{int(winner['Kriteria lolos'])}/7 kriteria, tetapi belum mencapai PF 1,50 "
            "dan drawdown maksimum 10% secara bersamaan."
        )
    st.info(
    ...
    st.caption(methodology["Execution contract"])
    st.caption(methodology["Threshold policy"])

    m1, m2, m3, m4, m5, m6 = st.columns(6)
    m1.metric("Kandidat terbaik", winner["Kandidat"])
    m2.metric(
        "PF development",
        f"{winner['PF development']:.3f}",
        f"{winner['PF development'] - control['PF development']:+.3f} vs control",
    )
    m3.metric(
        "DD development",
        f"{winner['DD development (%)']:.2f}%",
        f"{winner['DD development (%)'] - control['DD development (%)']:+.2f} pp",
        delta_color="inverse",
    )
    m4.metric("Growth development", f"{winner['Growth development (%)']:+.2f}%")
    m5.metric("Transaksi development", f"{int(winner['Transaksi development'])}")
    m6.metric("Fold profitable", f"{int(winner['Fold profitable'])}/12")
    ...
    with c1:
        ...
        st.dataframe(
            payload["thresholds"].style.format({"Nilai": "{:.6f}"}),
            use_container_width=True,
            hide_index=True,
        )
    with c2:
        st.markdown("**Retensi Sinyal**")
        st.dataframe(
            payload["retention"].style.format(
                {
    ...
    monte_carlo = payload["winner_monte_carlo_summary"]
    st.info(
        f"Monte Carlo kandidat terbaik: probabilitas equity akhir di bawah modal "
        f"**{monte_carlo['Probabilitas equity akhir < modal awal (%)']:.2f}%**, "
        f"equity P5 **USD {monte_carlo['Monte Carlo equity P5']:,.2f}**, dan "
        f"drawdown P95 **USD {monte_carlo['Monte Carlo drawdown P95']:,.2f}**."
    )
    st.markdown("**Kesimpulan Eksperimen**")
    st.write(
        f"{winner['Kandidat']} memperbaiki PF development dari "
        f"{control['PF development']:.3f} menjadi {winner['PF development']:.3f} dan "
        f"menurunkan drawdown dari {control['DD development (%)']:.2f}% menjadi "
        f"{winner['DD development (%)']:.2f}%. Perbaikan ini bermakna, tetapi target PF "
        "1,50 dan drawdown 10% belum tercapai. Kandidat belum menggantikan Fixed Delay 5m "
        "yang sedang menjalani paper live."
    )

    with st.expander("Detail fold, stress, bulanan, sinyal, dan audit data"):
        ...
        st.markdown(f"**Stress Development Kandidat Terbaik: {payload['winner']}**")
        st.dataframe(
            payload["winner_stress"].style.format(metric_formats, na_rep="-"),
            use_container_width=True,
            hide_index=True,
        )
        st.markdown("**Pertumbuhan Bulanan Kandidat Terbaik**")
        st.dataframe(
            payload["winner_monthly"].style.format(precision=3),
            use_container_width=True,
            hide_index=True,
        )
        st.markdown("**Audit Seleksi Sinyal**")
        st.dataframe(
            payload["signal_audit"].style.format({"Retensi (%)": "{:.1f}%"}),
            use_container_width=True,
            hide_index=True,
        )
        st.markdown("**Audit Data**")
...
def _render_v1_sell_specialist_v6_tab(payload) -> None:
    st.subheader("v1 SELL Specialist Lab v6 - Bear Regime & Setup Separation")
    if payload is None:
        st.warning("Hasil SELL Specialist v6 belum tersedia pada artefak precomputed.")
        return

    winner = payload["ranking"].iloc[0]
    methodology = payload["methodology"]
    st.warning(
        "**Eksperimen offline terisolasi:** Baseline v1, BUY Specialist v4, "
        "dan ledger paper live tidak berubah."
    )
    st.error(
        f"**SELL SPECIALIST v6 BELUM LULUS.** "
        f"**{payload['selection_status']}**. Peringkat heuristik tertinggi adalah "
        f"**{winner['Kandidat']}** ({int(winner['Kriteria lolos'])}/10), tetapi "
        f"development growth "
        f"**{winner['Growth development (%)']:+.2f}%**, PF "
        f"**{winner['PF development']:.3f}**, transaksi hanya "
        f"**{int(winner['Transaksi development'])}**, dan locked 2025 "
        f"**{winner['Growth locked 2025 (%)']:+.2f}%**."
    )
    st.info(
        "Hard bear-regime gate berhasil mengurangi sinyal koreksi bullish, tetapi "
        "outcome TP-before-SL pada periode threshold 2023H2 terlalu jarang. "
        "Model setup tidak memperoleh probabilitas dasar yang stabil, sehingga "
        "filter menjadi terlalu defensif atau tetap rugi ketika dilonggarkan."
    )
    st.caption(
        f"Train {methodology['Train']} | calibration "
        f"{methodology['Probability calibration']} dan "
    ...
    st.caption(methodology["Bear gate"])
    st.caption(methodology["Execution"])

    c1, c2, c3, c4, c5, c6 = st.columns(6)
    c1.metric("Kandidat selection", winner["Kandidat"])
    c2.metric("Growth development", f"{winner['Growth development (%)']:+.2f}%")
    c3.metric("Profit factor", f"{winner['PF development']:.3f}")
    c4.metric("Drawdown", f"{winner['DD development (%)']:.2f}%")
    c5.metric("Transaksi", f"{int(winner['Transaksi development'])}")
    ...
    st.markdown("**Peringkat Berdasarkan Model Selection 2024**")
    st.dataframe(
        payload["ranking"].style.format(ranking_formats, na_rep="-"),
        use_container_width=True,
        hide_index=True,
    ...
    with st.expander(
        "Detail regime, setup, model outcome, periode, fold, dan stress"
    ):
        st.markdown("**Audit Bear Regime dan Setup**")
        st.dataframe(
            payload["regime_setup_audit"].style.format(precision=2, na_rep="-"),
            use_container_width=True,
            hide_index=True,
        )
        st.markdown("**Kalibrasi Model Outcome per Setup**")
        st.dataframe(
            payload["model_selection"].style.format(precision=4, na_rep="-"),
            use_container_width=True,
            hide_index=True,
        )
        st.markdown("**Funnel Konfirmasi**")
        st.dataframe(payload["funnel"], use_container_width=True, hide_index=True)
        st.markdown("**Validasi Ekonomi per Periode**")
        st.dataframe(
            payload["period_validation"].style.format(metric_formats, na_rep="-"),
        ...
        st.markdown("**Fold, Monte Carlo, Konsentrasi, dan Stress**")
        st.dataframe(
            payload["folds"].style.format(metric_formats, na_rep="-"),
            use_container_width=True,
            hide_index=True,
        ...
        if not payload["v5_reference"].empty:
            st.markdown("**Referensi Kandidat Terbaik v5**")
            v5_formats = {
                column: formatter
                for column, formatter in ranking_formats.items()
                if column in payload["v5_reference"].columns
            }
            st.dataframe(
                payload["v5_reference"].style.format(
                    v5_formats, na_rep="-"
                ),
                use_container_width=True,
                hide_index=True,
...
def _render_v1_sell_specialist_v7_tab(payload) -> None:
    st.subheader("v1 SELL Specialist Lab v7 - Bear Event & Exhaustion Timing")
    if payload is None:
        st.warning("Hasil SELL Specialist v7 belum tersedia pada artefak precomputed.")
        return
    ...
    st.warning(
        "**Eksperimen offline terisolasi:** Baseline v1, BUY Specialist v4, "
        "dan seluruh ledger paper live tetap terkunci."
    )
    st.error(
        f"**SELL SPECIALIST v7 BELUM LULUS.** {payload['selection_status']}. "
        f"Kandidat tertinggi **{top['Kandidat']}** menghasilkan selection 2024 "
        f"**{top['Growth selection 2024 (%)']:+.2f}%** dari "
        f"**{int(top['Transaksi selection 2024'])} transaksi**, tetapi development "
        f"**{top['Growth development (%)']:+.2f}%**, PF "
        f"**{top['PF development']:.3f}**, DD "
        f"**{top['DD development (%)']:.2f}%**, dan locked 2025 "
        f"**{top['Growth locked 2025 (%)']:+.2f}%**."
    )
    st.info(
        "Event-level architecture mengurangi sinyal berulang dan berhasil menemukan "
        "edge lokal pada selection 2024. Namun precision locked kedua setup menjadi "
        "nol dan kandidat adaptive gagal pada seluruh sembilan stress scenario. "
        "Hasil ini belum layak masuk Paper Live Trading."
    )
    st.caption(methodology["Event detector"])
    st.caption(methodology["Phase"])
    st.caption(methodology["Execution"])

    c1, c2, c3, c4, c5, c6 = st.columns(6)
    c1.metric("Kandidat selection", top["Kandidat"])
    c2.metric("Selection 2024", f"{top['Growth selection 2024 (%)']:+.2f}%")
    c3.metric("Development", f"{top['Growth development (%)']:+.2f}%")
    c4.metric("Profit factor", f"{top['PF development']:.3f}")
    c5.metric("Drawdown", f"{top['DD development (%)']:.2f}%")
    c6.metric("Kriteria", f"{int(top['Kriteria lolos'])}/10")
    ...
    with st.expander("Detail event, fase, path, model, periode, dan risiko"):
        st.markdown("**Audit Bear Event dan Setup**")
        st.dataframe(
            payload["event_audit"].style.format(precision=2, na_rep="-"),
            use_container_width=True,
            hide_index=True,
        )
        st.markdown("**Distribusi Fase Event**")
        st.dataframe(payload["phase_audit"], use_container_width=True, hide_index=True)
        st.markdown("**Audit Path TP/SL, MFE, MAE, dan Time-to-TP**")
        st.dataframe(
            payload["path_audit"].style.format(precision=2, na_rep="-"),
            use_container_width=True,
            hide_index=True,
        )
        st.markdown("**Kalibrasi Outcome per Setup**")
        st.dataframe(
            payload["model_selection"].style.format(precision=4, na_rep="-"),
            use_container_width=True,
            hide_index=True,
        )
        st.markdown("**Klasifikasi Selection, Locked, dan Reference**")
        for label, key in (
...
def _render_v1_sideways_specialist_tab(payload) -> None:
    st.subheader("v1 Sideways Specialist Lab v1 - Range Detection & Mean Reversion")
    if payload is None:
        st.warning("Hasil Sideways Specialist belum tersedia pada artefak precomputed.")
        return
    ...
    st.warning(
        "**Eksperimen offline terisolasi:** Baseline v1, BUY Specialist v4, "
        "SELL Specialist, dan seluruh ledger paper live tetap terkunci."
    )
    st.error(
        f"**SIDEWAYS SPECIALIST BELUM LULUS.** {payload['selection_status']}. "
        f"Kandidat tertinggi **{top['Kandidat']}** menghasilkan selection 2024 "
        f"**{top['Growth selection 2024 (%)']:+.2f}%** dari "
        f"**{int(top['Transaksi selection 2024'])} transaksi** dan locked 2025 "
        f"**{top['Growth locked 2025 (%)']:+.2f}%**. Namun development "
        f"**{top['Growth development (%)']:+.2f}%**, PF "
        f"**{top['PF development']:.3f}**, DD "
        f"**{top['DD development (%)']:.2f}%**, dan 2026H1 "
        f"**{top['Growth 2026H1 (%)']:+.2f}%**."
    )
    st.info(
        "Range detector dan outcome model sudah menghasilkan BUY/SELL yang relatif "
        "seimbang serta positif pada selection dan locked. Edge bersih masih tipis: "
        "kandidat adaptive gagal pada seluruh sembilan stress scenario sehingga "
        "belum layak masuk Paper Live Trading."
    )
    st.caption(methodology["Range gate"])
    st.caption(methodology["Entry"])
    st.caption(methodology["Exit"])

    c1, c2, c3, c4, c5, c6 = st.columns(6)
    c1.metric("Kandidat selection", top["Kandidat"])
    c2.metric("Selection 2024", f"{top['Growth selection 2024 (%)']:+.2f}%")
    c3.metric("Locked 2025", f"{top['Growth locked 2025 (%)']:+.2f}%")
    c4.metric("Development PF", f"{top['PF development']:.3f}")
    c5.metric("Drawdown", f"{top['DD development (%)']:.2f}%")
    c6.metric("Kriteria", f"{int(top['Kriteria lolos'])}/10")
    ...
    with st.expander("Detail range, arah, path, model, periode, dan risiko"):
        st.markdown("**Audit Range Detector**")
        st.dataframe(
            payload["range_audit"].style.format(precision=2, na_rep="-"),
            use_container_width=True,
            hide_index=True,
        )
        st.markdown("**Opportunity BUY dan SELL**")
        st.dataframe(
            payload["opportunity_audit"].style.format(precision=2, na_rep="-"),
            use_container_width=True,
            hide_index=True,
        )
        st.markdown("**Audit Path MFE, MAE, dan Time-to-TP**")
        st.dataframe(
            payload["path_audit"].style.format(precision=2, na_rep="-"),
            use_container_width=True,
            hide_index=True,
        )
        st.markdown("**Kalibrasi Outcome Model**")
        st.dataframe(
            payload["model_selection"].style.format(precision=4, na_rep="-"),
            use_container_width=True,
            hide_index=True,
        )
        st.markdown("**Klasifikasi per Arah dan Periode**")
        for label, key in (
            ("Selection 2024", "classification_selection"),
            ("Locked 2025", "classification_locked"),
            ("Reference 2026H1", "classification_reference"),
...
def _render_v1_sideways_specialist_v2_tab(payload) -> None:
    st.subheader("v1 Sideways Specialist Lab v2 - Range Persistence & Breakout Hazard")
    if payload is None:
        st.warning("Hasil Sideways Specialist v2 belum tersedia.")
        return

    ranking = payload["ranking"]
    heuristic = ranking.iloc[0]
    eligible = ranking.loc[ranking["Kandidat"].eq(payload["winner"])]
    focus = eligible.iloc[0] if not eligible.empty else heuristic
    methodology = payload["methodology"]
    st.warning(
        "**Eksperimen offline terisolasi:** Sideways v1 menjadi control; seluruh "
        "strategi dan ledger Paper Live Trading tetap terkunci."
    )
    st.error(
        f"**SIDEWAYS SPECIALIST v2 BELUM LULUS.** {payload['selection_status']}. "
        f"Kandidat eligible **{focus['Kandidat']}** menghasilkan development "
        f"**{focus['Growth development (%)']:+.2f}%**, selection 2024 "
        f"**{focus['Growth selection 2024 (%)']:+.2f}%**, locked 2025 "
        f"**{focus['Growth locked 2025 (%)']:+.2f}%**, PF "
        f"**{focus['PF development']:.3f}**, dan DD "
        f"**{focus['DD development (%)']:.2f}%**."
    )
    st.success(
        "Breakout Hazard Gate lulus 9/9 stress scenario dan 6/8 primary folds. "
        "Kemajuan ini belum cukup untuk live karena PF masih di bawah 1.30, "
        "probabilitas rugi Monte Carlo 32.83%, dan 2026H1 masih negatif."
    )
    if heuristic["Kandidat"] != focus["Kandidat"]:
        st.info(
            f"Peringkat heuristik tertinggi adalah **{heuristic['Kandidat']}**, "
            f"tetapi hanya memiliki {int(heuristic['Transaksi selection 2024'])} "
            "transaksi selection sehingga tidak eligible."
        )
    st.caption(methodology["Persistence"])
    st.caption(methodology["Hazard"])
    st.caption(methodology["Directional"])

    c1, c2, c3, c4, c5, c6 = st.columns(6)
    c1.metric("Kandidat eligible", focus["Kandidat"])
    c2.metric("Development", f"{focus['Growth development (%)']:+.2f}%")
    c3.metric("Selection 2024", f"{focus['Growth selection 2024 (%)']:+.2f}%")
    c4.metric("Locked 2025", f"{focus['Growth locked 2025 (%)']:+.2f}%")
    c5.metric("Drawdown", f"{focus['DD development (%)']:.2f}%")
    c6.metric("Kriteria", f"{int(focus['Kriteria lolos'])}/10")
    ...
    ranking_formats = {
        ...
        "Transaksi development": "{:.0f}",
        "Growth locked 2025 (%)": "{:+.2f}%",
        "Persistence precision locked": "{:.1%}",
        "Hazard precision locked": "{:.1%}",
        "Growth 2026H1 (%)": "{:+.2f}%",
    }
    metric_formats = {
        "Sinyal tersedia": "{:.0f}",
    ...
    with st.expander("Detail label, model, arah, sesi, periode, dan risiko"):
        st.markdown("**Audit Label Persistence dan Breakout**")
        st.dataframe(
            payload["label_audit"].style.format(precision=2, na_rep="-"),
            use_container_width=True,
            hide_index=True,
        )
        st.markdown("**Model Outcome, Persistence, dan Hazard**")
        for key in (
            "outcome_model_selection",
            "persistence_model_selection",
            "hazard_model_selection",
        ):
            st.dataframe(
                payload[key].style.format(precision=4, na_rep="-"),
                use_container_width=True,
        ...
        st.markdown("**Threshold Arah dan Audit Sesi**")
        st.dataframe(
            payload["directional_thresholds"].style.format(
                precision=4, na_rep="-"
            ),
            use_container_width=True,
            hide_index=True,
        ...
        st.dataframe(
            payload["session_audit"].style.format(precision=2, na_rep="-"),
            use_container_width=True,
            hide_index=True,
        )
        st.markdown("**Klasifikasi Selection, Locked, dan Reference**")
...
def _render_v1_sideways_specialist_v4_tab(payload) -> None:
    st.subheader(
        "v1 Sideways Specialist Lab v4 - Event-Based Hazard & Selective Protection"
    )
    if payload is None:
        st.warning("Hasil Sideways Specialist v4 belum tersedia.")
        return
    ...
    best_experiment = ranking.loc[
        ~ranking["Kandidat"].eq("Breakout Hazard v2 Control")
    ].iloc[0]
    st.warning(
        "**Eksperimen offline terisolasi:** entry Breakout Hazard Gate v2 "
        "identik. Model episode hanya mengubah keputusan exit dan tidak "
        "menyentuh ledger Paper Live Trading."
    )
    st.error(
        "**SIDEWAYS SPECIALIST v4 BELUM LULUS.** Control v2 tetap peringkat "
        f"pertama dengan development **{control['Growth development (%)']:+.2f}%** "
        f"dan **{int(control['Kriteria lolos'])}/11** kriteria. Kandidat event "
        f"terbaik, **{best_experiment['Kandidat']}**, menghasilkan development "
        f"**{best_experiment['Growth development (%)']:+.2f}%**, PF "
        f"**{best_experiment['PF development']:.3f}**, DD "
        f"**{best_experiment['DD development (%)']:.2f}%**, dan reference "
        f"2026H1 **{best_experiment['Growth 2026H1 (%)']:+.2f}%**."
    )
    st.info(
        "Dataset episode memperbaiki effective sample size, tetapi threshold "
        "masih terlalu sensitif terhadap hazard semu. Selective Protection "
        "mencatat net exit benefit positif, namun belum stabil pada selection "
        "2024 dan reference 2026H1."
    )

    c1, c2, c3, c4, c5, c6 = st.columns(6)
    c1.metric("Pemenang", control["Kandidat"])
    c2.metric("Control development", f"{control['Growth development (%)']:+.2f}%")
    c3.metric(
        "Event terbaik", best_experiment["Kandidat"]
    )
    c4.metric(
        "Event development",
        f"{best_experiment['Growth development (%)']:+.2f}%",
    )
    c5.metric(
        "Event drawdown", f"{best_experiment['DD development (%)']:.2f}%"
    )
    c6.metric("Status", "Belum lulus")
    ...
    st.markdown("**Saved Loss vs Sacrificed Profit**")
    st.dataframe(
        payload["exit_attribution"].style.format(
            {
                "Common entry": "{:.0f}",
    ...
    with st.expander("Detail episode, model, periode, fold, risiko, dan stress"):
        for key, value in payload["methodology"].items():
            if key != "Name":
                st.caption(f"**{key}:** {value}")
        st.markdown("**Audit Episode Counterfactual**")
        st.dataframe(
            payload["episode_audit"].style.format(precision=2, na_rep="-"),
            use_container_width=True,
            hide_index=True,
        )
        st.markdown("**Kalibrasi Event Hazard**")
        st.dataframe(
            payload["event_model_selection"].style.format(
                precision=4, na_rep="-"
            ),
            use_container_width=True,
            hide_index=True,
        ...
        for label, key in (
            ...
            st.markdown(f"**Klasifikasi {label}**")
            st.dataframe(
                payload[key].style.format(precision=4, na_rep="-"),
                use_container_width=True,
                hide_index=True,
        ...
        st.markdown("**Referensi Peringkat v3**")
        st.dataframe(
            payload["v3_reference"].style.format(precision=3, na_rep="-"),
            use_container_width=True,
            hide_index=True,
        )
        st.markdown("**Audit Data**")
...
def _render_v1_sideways_specialist_v7_tab(payload) -> None:
    st.subheader(
        "v1 Sideways Specialist v7 - MTF Regime, Native Persistence & Rejection Entry"
    )
    if payload is None:
        st.warning("Hasil Sideways Specialist v7 belum tersedia.")
        return

    ranking = payload["ranking"]
    control = ranking.loc[ranking["Kandidat"].eq(payload["control"])].iloc[0]
    experimental = ranking.loc[
        ~ranking["Kandidat"].eq(payload["control"])
    ].iloc[0]
    st.warning(
        "**Eksperimen entry terisolasi:** execution engine v6 dikunci. H1 hanya "
        "memberi izin rezim, indikator persistence dihitung dari candle M15 selesai, "
        "dan M1 tetap menjadi waktu entry. Ledger Paper Live Trading tidak disentuh."
    )
    st.error(
        "**BELUM LULUS:** native M15 dan fusion terlalu agresif menyaring sinyal. "
        f"Kontrol tetap memimpin dengan growth development "
        f"**{control['Growth development (%)']:+.2f}%**. Kandidat baru terbaik, "
        f"**{experimental['Kandidat']}**, hanya mempertahankan "
        f"**{experimental['Retensi sinyal (%)']:.1f}%** sinyal dan mencatat "
        f"reference 2026H1 **{experimental['Growth 2026H1 (%)']:+.2f}%**."
    )
    st.info(
        "Temuan penting: pemisahan timeframe valid secara arsitektur, tetapi ambang "
        "kuantil H1 dan M15 tidak boleh ditumpuk sebagai hard gate. Fusion penuh "
        "menghasilkan nol transaksi, sehingga belum ada bukti untuk mengganti v6."
    )

    c1, c2, c3, c4, c5, c6 = st.columns(6)
    c1.metric("Pemenang", payload["winner"])
    c2.metric("Growth development", f"{control['Growth development (%)']:+.2f}%")
    c3.metric("Profit factor", f"{control['PF development']:.3f}")
    c4.metric("Max drawdown", f"{control['DD development (%)']:.2f}%")
    c5.metric("Reference 2026H1", f"{control['Growth 2026H1 (%)']:+.2f}%")
    c6.metric("Kandidat baru terbaik", str(experimental["Kandidat"]))
    ...
    with st.expander("Metodologi, threshold native, validasi, dan risiko"):
        for key, value in payload["methodology"].items():
            if key != "Name":
                st.caption(f"**{key}:** {value}")
        st.json(payload["native_m15_thresholds"])
        for label, key in (
            ("Validasi Ekonomi per Periode", "period_validation"),
            ("Historical Reference 2026H1", "historical_reference"),
            ("Fold", "folds"),
...
def _render_v1_sideways_specialist_v9_tab(payload) -> None:
    ...
    if payload is None:
        st.warning("Hasil Sideways Specialist v9 belum tersedia.")
        return

    ranking = payload["ranking"].copy()
    winner_name = payload["winner"] or str(ranking.iloc[0]["Kandidat"])
    winner = ranking.loc[ranking["Kandidat"].eq(winner_name)].iloc[0]
    moderate = ranking.loc[ranking["Kandidat"].eq("Moderate Regime")].iloc[0]
    st.warning(
        "**Eksperimen kelonggaran terkontrol:** regime diperluas menjadi minimal "
        "2/5 bukti dan persistensi satu M15. Hazard ekstrem tetap menjadi veto. "
        "BUY Specialist v4 dan ledger paper live tidak disentuh."
    )
    st.error(
        "**BELUM LULUS.** Opportunity Expansion Full memimpin selection 2024, "
        "tetapi PF development dan drawdown belum memenuhi batas. Moderate Regime "
        f"lebih stabil lintas periode dengan growth development "
        f"**{moderate['Growth development (%)']:+.2f}%**, locked 2025 "
        f"**{moderate['Growth locked 2025 (%)']:+.2f}%**, dan 2026H1 "
        f"**{moderate['Growth 2026H1 (%)']:+.2f}%**, tetapi PF development baru "
        f"**{moderate['PF development']:.3f}**."
    )
    st.info(
        "Kelonggaran berhasil membuktikan bahwa peluang tambahan memang ada: "
        "Moderate Regime menambah transaksi lebih dari 12 kali dengan marginal PF "
    ...
    st.markdown("**Kualitas Transaksi Tambahan**")
    st.dataframe(
        payload["marginal_trade_audit"].style.format(
            {
                "Kenaikan transaksi (%)": "{:+.1f}%",
                "Marginal growth (%)": "{:+.2f}%",
                "Marginal PF": "{:.3f}",
                "Marginal win rate (%)": "{:.1f}%",
            },
            na_rep="-",
        ),
        use_container_width=True,
    ...
    st.dataframe(
        payload["filter_audit"].style.format(
            {"Retensi opportunity (%)": "{:.1f}%", "Median RR": "{:.3f}"},
            na_rep="-",
        ),
        use_container_width=True,
        hide_index=True,
    ...
    with st.expander("Metodologi, threshold, validasi, dan risiko"):
        for key, value in payload["methodology"].items():
            if key != "Name":
                st.caption(f"**{key}:** {value}")
        st.json(payload["expansion_thresholds"])
        for label, key in (
            ("Validasi Ekonomi per Periode", "period_validation"),
            ("Historical Reference 2026H1", "historical_reference"),
            ("Fold", "folds"),
...
def render_simulation(
    ...
    with sell_specialist_v6_tab:
        _render_v1_sell_specialist_v6_tab(
            load_precomputed_v1_sell_specialist_v6(
                V1_SELL_SPECIALIST_V6_VERSION
            )
        )
    with sell_specialist_v7_tab:
        _render_v1_sell_specialist_v7_tab(
            load_precomputed_v1_sell_specialist_v7(
                V1_SELL_SPECIALIST_V7_VERSION
            )
        )
    with sideways_specialist_v1_tab:
        _render_v1_sideways_specialist_tab(
            load_precomputed_v1_sideways_specialist(
                V1_SIDEWAYS_SPECIALIST_VERSION
            )
        )
    with sideways_specialist_v2_tab:
    ...
    with sideways_specialist_v4_tab:
    ...
    with sideways_specialist_v7_tab:
    ...
    with sideways_specialist_v9_tab:
        _render_v1_sideways_specialist_v9_tab(
            load_precomputed_v1_sideways_specialist_v9(
                V1_SIDEWAYS_SPECIALIST_V9_VERSION
            )
        )
