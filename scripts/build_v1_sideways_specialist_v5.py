...
from gold_forecast.v1_sideways_specialist_v5 import (
    run_v1_sideways_specialist_v5_lab,
)
...
OUTPUT_PATH = (
    PROJECT_ROOT / "data" / "precomputed" / "v1_sideways_specialist_v5.pkl.b64"
)
V4_PATH = PROJECT_ROOT / "data" / "precomputed" / "v1_sideways_specialist_v4.pkl.b64"
VERSION = "optimizer-v1-sideways-specialist-exit-attribution-2022-2026h1-v5"
...
def main() -> None:
    ...
    v4_payload = None
    if V4_PATH.exists():
        artifact = pickle.loads(base64.b64decode(V4_PATH.read_bytes()))
        v4_payload = artifact.get("payload")
    payload = run_v1_sideways_specialist_v5_lab(
        gold_m1, frozen, v4_payload
    )
    ...
    print(
        f"Sideways Specialist v5 selesai | selection={payload['selection_status']} | "
        f"top={top['Kandidat']} | criteria={top['Kriteria lolos']}/"
        f"{payload['decisions'].iloc[0]['Total kriteria']} | "
