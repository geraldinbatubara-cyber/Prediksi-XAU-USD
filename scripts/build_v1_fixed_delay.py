...
from gold_forecast.v1_fixed_delay import run_v1_fixed_delay_lab
...
OUTPUT_PATH = PROJECT_ROOT / "data" / "precomputed" / "v1_fixed_delay.pkl.b64"
VERSION = "optimizer-v1-fixed-delay-5m-robustness-2022-2026h1-v1"
...
def main() -> None:
    ...
    if gold_m1.empty:
        raise RuntimeError(
            "Cache M1 2021-10 sampai 2026-06 belum lengkap. "
            "Lengkapi data sebelum membangun lab."
        )
    ...
    payload = run_v1_fixed_delay_lab(
    ...
    print(
        f"Fixed Delay 5m selesai | growth={result['Growth (%)']:.2f}% | "
        f"PF={result['Profit factor']:.3f} | DD={result['Max drawdown (%)']:.2f}% | "
        f"tx={result['Transaksi']:.0f} | "
        f"fold={payload['decision']['Jumlah fold profitable']}/12 | "
