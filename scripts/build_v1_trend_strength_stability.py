...
from gold_forecast.v1_trend_strength_stability import (
    run_v1_trend_strength_stability_lab,
)
...
VERSION = "optimizer-v1-trend-strength-stability-2022-2026h1-v1"
...
def main() -> None:
    ...
    payload = run_v1_trend_strength_stability_lab(
        gold_m1,
    ...
    print(
        f"Trend Strength Stability selesai | winner={winner['Kandidat']} | "
        f"dev growth={winner['Growth development (%)']:.2f}% | "
        f"dev PF={winner['PF development']:.3f} | "
        f"dev DD={winner['DD development (%)']:.2f}% | "
        f"criteria={winner['Kriteria lolos']}/8 | passed={winner['Lulus']} | "
        f"stability={payload['stability']['Interpretasi']}"
    )
