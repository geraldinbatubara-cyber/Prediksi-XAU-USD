...
from gold_forecast.v1_entry_outcome import run_v1_entry_outcome_lab


INPUT_DIR = PROJECT_ROOT / "data" / "intraday"
OOS_SOURCE = PROJECT_ROOT / "data" / "precomputed" / "optimizer_oos.pkl"
OUTPUT_PATH = PROJECT_ROOT / "data" / "precomputed" / "v1_entry_outcome.pkl.b64"
VERSION = "optimizer-v1-balanced-entry-outcome-2025-2026h1-v1"
...
def main() -> None:
    ...
    payload = run_v1_entry_outcome_lab(gold_m1, load_gold_data(), frozen)
    ...
    economic = payload["economic"].set_index("Strategi").loc["v1 Probability-Gated Entry"]
    print(
        f"Entry Outcome Lab selesai | model={methodology['Selected model']} | "
        f"threshold={methodology['Selected threshold']:.0%} | "
        f"Brier improvement={validation['Brier improvement (%)']:.2f}% | "
        f"growth={economic['Growth (%)']:.2f}% | PF={economic['Profit factor']:.3f} | "
        f"DD={economic['Max drawdown (%)']:.2f}% | "
