...
from gold_forecast.v1_regime_classifier_v3 import (
    run_v1_regime_classifier_v3_lab,
)
...
OUTPUT_PATH = (
    PROJECT_ROOT / "data" / "precomputed" / "v1_regime_classifier_v3.pkl.b64"
)
VERSION = "optimizer-v1-regime-classifier-hierarchical-soft-gate-2022-2026h1-v3"
...
def main() -> None:
    ...
    payload = run_v1_regime_classifier_v3_lab(
        gold_m1,
    ...
    print(
        f"Regime Classifier v3 selesai | winner={winner['Kandidat']} | "
        f"classifier={winner['Kriteria classifier lolos']}/6 | "
        f"economic={winner['Kriteria ekonomi lolos']}/9 | "
        f"growth={winner['Growth development (%)']:.2f}% | "
        f"PF={winner['PF development']:.3f} | "
        f"DD={winner['DD development (%)']:.2f}% | "
        f"passed={winner['Lulus']}"
    )
