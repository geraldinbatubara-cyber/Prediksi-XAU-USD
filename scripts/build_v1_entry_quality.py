...
from gold_forecast.v1_entry_quality import run_v1_entry_quality_lab


INPUT_DIR = PROJECT_ROOT / "data" / "intraday"
OOS_SOURCE = PROJECT_ROOT / "data" / "precomputed" / "optimizer_oos.pkl"
OUTPUT_PATH = PROJECT_ROOT / "data" / "precomputed" / "v1_entry_quality.pkl.b64"
VERSION = "optimizer-v1-entry-quality-2024-2026h1-v2"
DOWNLOAD_START = pd.Timestamp("2023-10-01")
...
def main() -> None:
    ...
    payload = run_v1_entry_quality_lab(gold_m1, signal_daily, frozen)
    ...
    print(
        f"Entry Quality Lab v2 selesai | model={methodology['Selected model']} | "
...
def _audit_monthly_coverage(data: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for period in pd.period_range("2024-01", "2026-06", freq="M"):
        month = data.loc[
