...
from gold_forecast.v1_sideways_defense import run_v1_sideways_defense_lab


INPUT_DIR = PROJECT_ROOT / "data" / "intraday"
OOS_SOURCE = PROJECT_ROOT / "data" / "precomputed" / "optimizer_oos.pkl"
OUTPUT_PATH = PROJECT_ROOT / "data" / "precomputed" / "v1_sideways_defense.pkl.b64"
VERSION = "optimizer-v1-sideways-defense-2025-2026h1-v1"
...
def main() -> None:
    ...
    payload = run_v1_sideways_defense_lab(gold_m1, load_gold_data(), frozen)
    ...
    row = payload["strategy_validation"].set_index("Strategi").loc["Hybrid Regime Strategy"]
    print(
        f"Sideways Defense selesai | classifier={payload['methodology']['Selected classifier']} | "
        f"sideways={payload['methodology']['Selected sideways config']} | "
        f"growth={row['Growth (%)']:.2f}% | PF={row['Profit factor']:.3f} | "
        f"DD={row['Max drawdown (%)']:.2f}% | passed={payload['decision']['Lulus seluruh kriteria']}"
    )
